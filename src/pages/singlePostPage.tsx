...
import { managePageStateRendring, PostPageState } from "@/utils/utils";
...
export default function SinglePostPage() {
	...
	const [state, setState] = useState<PostPageState>({ status: "loading" });
	...
	useEffect(() => {
		async function fetchSinglePost() {
			...
			setState({ status: "loaded", post: postsData as Post });

			if (!postsData.allow_comments) {
						...
						{post.type === "belief" && post.metadata?.evo_history?.length > 1 && (
							...
							>
								{showOldBeliefs ? (
									...
									<div>
										View previous +{post.metadata?.evo_history?.length - 1}
									</div>
								)}
							</button>
						)}
